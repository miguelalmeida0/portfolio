...
Scope: Miguel Design OS as a local agent operating system for frontend generation, visual target reconstruction, screenshot QA, skill routing, artifact validation, and embedded `docs/internal/design-system` use.
...
- Recent embedded `docs/internal/design-system` support lets the CLI resolve internal Design OS files from its own root instead of the host project root.
...
| Ease of use inside another project | 8 | Embedded `docs/internal/design-system` support materially improves portability. |
