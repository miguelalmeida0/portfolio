...
## Embedded `docs/internal/design-system` Usage

When Miguel Design OS is copied into another project as `docs/internal/design-system`, run the CLI from that project root:
...
The CLI resolves internal Design OS paths from the location of `docs/internal/design-system/tools/design-os.mjs`, so `skills/skill-registry.json`, `schemas/`, `templates/`, `evaluation/`, and `design-dna/` are loaded from `docs/internal/design-system` instead of the host project root.
